...
// matris jacobiana do sistema
void jacob (double *y, double J[NN][NN]) {
  ...
  J[0][2] = (r3*y[2]/(y[0]+k3))*(1.0e0-y[0]/(y[0]+k3)) - a31*y[2] - a32*y[3];
  J[1][2] = 0.0e0;
...
void multi_mat (double *y, double *ydot, double J[NN][NN]) {
  int i, j, k, A, D;
  ...
  for (j = 0; j < NN; j++) {
    for (i = 0; i < NN; i++) {
      A = (i + 1) * NN + j;                                         // ajusta elemento de vdot
      ydot[A] = 0.e0;                                              // zera vdot p/ somatorio
      for (k = 0; k < NN; k++) {
        D = (k + 1) * NN + j;                                       //  ajusta elemento de v
        ydot[A] += J[k][i] * y[D];
      }
    }
  }
...
void derivs (int *nvars, double *t, double *y, double *ydot) {
